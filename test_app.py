...
#             st.error(f"[INIT] Error loading model: {e}")
#             raise
#         try:
#             self.client = OpenAI(base_url="https://router.huggingface.co/v1", api_key=api_key)
#         except Exception as e:
#             st.error(f"[INIT] Error creating OpenAI client: {e}")
#             raise
...
#             return img_array
#         except Exception as e:
#             st.error(f"[PREPROCESS] Error preprocessing image: {e}")
#             raise
...
#         except Exception as e:
#             st.error(f"[DETECT] Error during aircraft detection: {e}")
#             raise
...
#         except Exception as e:
#             st.error(f"[RUN_MODEL] Error generating description: {e}")
#             raise
...
#         except Exception as e:
#             st.error(f"[DESCRIPTION] Error generating description: {e}")
#             raise
...
#             return audio_stream
#         except Exception as e:
#             st.error(f"[TTS] Error during text-to-speech conversion: {e}")
#             return None
...
#         except Exception as e:
#             st.error(f"[MAIN] Error in prediction pipeline: {str(e)}")


import h5py
with h5py.File("aircraft_detector_model.h5", "r") as f:
    print(list(f.keys()))